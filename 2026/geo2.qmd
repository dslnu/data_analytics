...
format: 
  revealjs:
    ...
    multiplex:
      ...
      secret: '5553a1fd4a29f855e89152fb80d437f9'
      id: '7d4561b8d9080fc1cf58e09d2ed1bc68e118d857058b0fa9f1c19b1f01bb38f1'
---
...
## Common geometric operations {.scrollable .font8}
:::{.callout-tip icon=false}
## Read census data
```{python}
#| echo: true
...
## Common geometric operations {.scrollable}
:::{.callout-tip icon=false}
## Check geometries
...
1. The `.union_all()` returns a single geometry object:
...
2. The `.dissolve()` method returns a `GeoDataFrame` as an output (with aggregated attribute information).
...
## Common geometric operations {.font8}
:::{.callout-tip icon=false}
...
We will use [PROJ library](https://proj.org/)  which can be used through the [pyproj](https://pyproj4.github.io/pyproj/stable/) Python library.
...
## Working with map projections {.scrollable .font8}
:::{.callout-note icon=false}
...
Pick some CRS, e.g. [ETRS89 / TM35FIN (EPSG:3067)](https://spatialreference.org/ref/epsg/3067/). And pick an EU country of your choice (e.g. Finland)
...
For doing this, we can specify the CRS using a proj-string and specify the center point with a few of CRS parameters: `+lat`, `+lon` (see [PROJ documentation](https://proj.org/operations/projections/ortho.html) for details).    
...
geo = geocode(
    data["addr"], provider="nominatim", user_agent="quarto_lnu2", timeout=10
)
geo.head()
```
:::
