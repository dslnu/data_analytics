...
It is accompanied by spatial information that connects the data to a particular locationi
...
:::
![](../img/raster/continuous-elevation-HARV-plot-01.png)
...
:::{.callout-tip icon=false}
## Temporal dimensions
You may have longitudinal (repetitive) observations from the same area, constituting time series data. More often than not, you need to combine information from these layers to be able to conduct meaningful analysis based on the data, such as do a weather forecast.
...
:::{.callout-tip icon=false}
## Surface maps
A 3D surface map is a three-dimensional representation of a terrain or surface, created by plotting elevation or depth data as a continuous surface. Creating a 3D surface map can be done by calling the `.plot.surface()` method in `xarray`:
...
The information about NoData can be can be stored with different keys, and you should look for values stored in keys, such as `_FillValue`, `missing_value`, `fill_value` or `nodata`. In our raster file, this metadata has not been stored as part of the file, and thus, we cannot access the `.attrs` attribute:
