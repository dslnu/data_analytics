---
title: "Big Data: GeoScience part 5"
subtitle: "Geographic Data Science — Part I: Building Blocks"
author: 
  - name: MSDE
...
**Networks** — must account for **topology**, the structure of connections between nodes. We cannot assume every node is connected, and connections cannot be derived from the nodes alone.

*Example:* two subway stations may be far apart but linked by a frequent express train. Raw distance is then a poor indication of their true geographic relationship.
...
Structures arise from how processes are conceptualised **and** what questions the analyst seeks to answer.

Since measurement is often beyond the analyst's control:

> *how a geographical process actually operates* (its "causal" or "generative" structure)
> can be different from
> *how we are actually able to measure it*.
:::
...
This is not necessarily bad — embedding complex ideas in software widens the reach of a discipline. Desktop GIS in the 1990s–2000s made geographic information useful to a much wider audience, largely by standardizing the "geographic matrix" (Berry, 1964), which we now call a **geotable**.
:::
...
- exploratory statistics of spatial autocorrelation (Chs. 6 and 7)
- regionalization (Ch. 10)
- spatial econometrics (Ch. 11)
:::

:::{.callout-warning icon=false}
## One structure, many names

Ironically, several fields in computer science and mathematics came up with their own terminology for similar structures:
...
In this book: `networkX` graph objects, and the spatial weights objects in `pysal`, which rely on **sparse adjacency matrices** from `scipy`.
:::
...
- Interested in the **exact shape, length and location** of each segment?
  → a collection of independent lines/polygons that happen to touch at their ends → **geographic table**
...
## Connecting conceptual to computational
...
- fields → **rasters**
- objects → **vector-based tables**
- networks → *no first-class structure*; computed on the fly
...
→ The **"costs" of moving between data structures have been lowered**, reducing the importance of picking the "right" representation from the outset.
:::
...
Scientific results stress the issues of a *one-size-fits-all* scale of analysis, and show that results often **do** change when the geographical scale of analysis changes.

This may not matter in practical applications where **explanation is less important than prediction** — but it matters when it does.

> Yes, we *can* grid this — but *should* we?
...
- **Objects** → "vector" structures: points, lines, polygons
- **Fields** → "raster" structures: *pixels* record values, *bands* record the kind of data
- **Networks** → historically core to data *models*, poorly served by classic GIS data *structures* — this is changing

By recognizing that the **conceptual data model can be distinct from the computational data structure**, we can move more easily between structures.

Be attentive to what the entities you analyse *are*, and aware of when that is at odds with *how they are measured*.
:::
...
The scientific and computational context in which the book is framed.

1. Debates around **Open Science**, its origins, and how the computational community responds
2. Three innovations: **computational notebooks**, **open-source packages**, **reproducible platforms**
3. A practical introduction to the key infrastructure: Jupyter / JupyterLab, Python packages, and a containerized platform
:::
...
> The scientific process, at its core, is meant to be **transparent and accessible to anyone**.

Openness is not an "add-on" making cosmetic changes to the scientific approach — it is a key component of what makes our practices *Science*.

Science "stands on the shoulders of giants" and progresses through dialectics. That can only work if the community can access and study both **results** *and* **the process that created them**.

**Transparency, accessibility, inclusiveness** are critical for good science.
:::

:::{.callout-note icon=false}
## A short history

Openness was a core commitment of early scientists — one of the key differences from their contemporary *alchemists*, who worked on similar topics in a much more opaque way.

Scientists recorded field or lab experiments in paper notebooks and diaries:

- to remember what they had done and how they arrived at their results
- to ensure other members of the community could **study, understand and replicate** their findings
:::

## Open science

![Galileo's drawings of Jupiter and the Medicean stars — the power of diagrams inside scientific texts](../img/gds/medicean_stars.png){width="26%" fig-align="center"}
...
There is a growing perception that this original scientific ethos has been lost. High-profile scandals have prompted some to call a state of **crisis** (Ioannidis, 2007).

The "crisis" arises because the analyses scientists conduct are **difficult to repeat**. Sometimes it is even impossible to clearly understand the steps taken to arrive at results.
:::

:::{.callout-tip icon=false}
## Why? Technology plays a role

The original scientists relied on **analog** technologies, for which a parallel set of analog tools was developed to track procedures — paper notebooks where biologists drew species, or chemists detailed each step in the lab.

Social science had it easier: quantitative data was scarce, analysis relied on math to minimize computation, or on small datasets documented in the publication itself.

Today, much of the experimental workflow is dominated by **computers**. Writing every step down in a paper notebook **separates from the medium** in which those steps actually take place.
:::
...
> Scientific results need to be accompanied by enough information and detail so that they could be **repeated by a third party**.

Since much of modern science is mediated through computers, reproducibility poses important challenges for the computational tools and practices the scientific community builds and relies on.
:::
...
1. **Computational notebooks** — the top layer; capture the detail specific to a given project
2. **Open source packages** — support the code written in notebooks
3. **Reproducible platforms** — make it possible to transfer computations across hardware devices and architectures
:::
...
//| fig-width: 6
digraph stack {
  rankdir=BT; node [shape=box, style="rounded,filled", fontname="Helvetica", fillcolor="#EEF2FF", color="#4B58A6"];
  platform [label="Reproducible platforms\n(Docker, containers)"];
  packages [label="Open source packages\n(geopandas, pysal, xarray)"];
  notebooks [label="Computational notebooks\n(Jupyter)"];
  platform -> packages -> notebooks [color="#4B58A6"];
}
```
...
:::{.callout-tip icon=false}
## The twenty-first century sibling of Galileo's notebooks

Like their predecessors, they let researchers record their practices and steps as they go about their work — but they are **fully integrated in the technological paradigm** in which research takes place today.

They are rapidly becoming the modern-day version of the traditional academic paper: the main vehicle on which computational knowledge is created, shared, and consumed.

They are also spreading into industry practice, e.g. in reports.
:::

:::{.callout-important icon=false}
## Three core features

1. **Single file** — narrative text, computer code, *and* the output produced by that code. The entire workflow is recorded and documented in one place, without ancillary devices.

2. **Interactive work** — modern computational work benefits from the ability to try, fail, tinker and iterate quickly. The same interface works on a laptop or a data centre, lowering the cognitive load of scaling up.

3. **Interoperability** — notebooks are designed to be easily convertible into other formats (e.g. PDF), so they can be read, printed and annotated without technical software.
:::
...
For notebooks to be an efficient medium, they must be **concise and streamlined**: include only what is unique to the application being recorded, and avoid duplication.

Code used several times across notebooks should be taken out and put into a **centralized place** — a package.

Packages are **modular, flexible, reusable** compilations of code. They do not capture specific applications but **abstractions of functionality**.
...
Open source software provides its own code for the user to inspect. The user can then modify it to make new software, or redistribute it.

Open source packages fulfil the same modularizing function as any package, but they also enable **transparency**: any user can access the exposed functionality *and* the underlying code that generates it.

For code packages to serve Open Science and reproducibility, they need to be **open source**.
:::
...
It is not enough to specify the code (notebooks) or even the shared functionality (packages) — the **computational environment** in which they are executed needs to be reproducible too.

> It is no longer enough for an analysis to just work on the author's laptop; it needs to work on "any laptop".

A reproducible platform specifies:

- lower-level infrastructure: operating system, sometimes hardware (e.g. GPUs)
- the **versions** of all packages required — changes to packages can change results or break workflows entirely
:::

:::{.callout-note icon=false}
## Containers

The notion of reproducible platforms is not as widespread or agreed upon as notebooks or packaging — its necessity has only become apparent recently.

**Containers** are a lightweight version of a virtual machine: they encapsulate an entire environment (or platform) in a format easy to transfer and reproduce in a variety of computational contexts.
...
A Jupyter notebook is a plain text file with the `.ipynb` extension — easy to move around, sync and track over time.

Internally it is a plain-text document containing **JSON** that records the state of the notebook, so it integrates well with modern web technologies.

Other notebook systems: Apache **Zeppelin**, the Alan Turing Institute's **Wrattler**, **ObservableHQ** — all generally web-based.
:::

:::{.callout-warning icon=false}
## Common traits (and a caveat)

- They separate the system that **executes** analytical code from the system that **displays** the document
- Many (Jupyter included) allow computational steps to be (re)run repeatedly **out of the order in which they appear**
  - great for experimenting directly with the code
  - **confusing** when code is executed out of order
- Their web-focused nature allows direct integration of visualizations and tables within the document
:::
...
A notebook is a **collection of cells** that, when executed in the correct order, describe and conduct an analysis.

**Text cells** contain Markdown — a popular set of rules to create rich content (headers, lists, links) from plain text, designed so that the plain text version looks similar to the output.

For more demanding tasks, text cells can integrate $\LaTeX$ notation: most narrative in Markdown, equations in $\LaTeX$.
:::

:::{.callout-tip icon=false}
## Code cells

Code cells are boxes that contain snippets of computer code. In this course all code is Python, but Jupyter is flexible enough for other languages via **kernels**.

```{python}
# This is a code cell
```

A code cell can be **run** to execute the code it contains. If the code produces an output (a table, a figure), it appears below the cell.

Every run increments the cell's counter, which indicates the order in which cells were run.
:::
...
The `IPython` package provides methods to bring a series of media directly into the notebook environment:

```{python}
import IPython.display as display
```

This opens the door to a large number of elements from the web — for example a YouTube clip:

```python
display.YouTubeVideo("iinQDhsdE9s")
```

or an `iframe` embedding an interactive OpenStreetMap map.
:::
...
And local images with `display.Image(path, width=250)`.
:::

## JupyterLab

:::{.callout-tip icon=false}
## An interface to the Jupyter ecosystem

JupyterLab brings together several data science tools into a consistent interface. It is a **web app following a client-server architecture** — the computation is *decoupled* from the interface.

This decoupling lets each be hosted wherever is most convenient:

- kernel and browser both on your laptop
- **or** the kernel in a cloud data centre, with JupyterLab open on a tablet
:::

## JupyterLab

![An annotated view of the JupyterLab interface](../img/gds/jupyter_lab.png){width="70%" fig-align="center"}

:::{.callout-note icon=false}
## Three main areas

- **Menu bar** (red) — open, create and interact with files; modify the appearance and behaviour of JupyterLab
- **Main pane** (blue) — the largest real estate. New notebook, console, terminal session, Markdown text file, contextual help. Open as many windows as needed and rearrange them by dragging and dropping
- **Side pane** (green) — tabs that toggle auxiliary information: a file browser, the list of open kernels and terminals, the **command palette**, the list of open windows
:::
...
From satellites controlled by NASA to economics courses by Nobel Prize-winning professors, Python is a fundamental component of "consensus" data science.

- good for **beginners** and **high-performance science** alike
- a vibrant scientific community (`scipy`, PyData) in universities *and* companies
- new methods and usability improvements released continuously
- in the geographic domain: the scripting language of **both** ArcGIS (proprietary) and QGIS (open)

Whether you are thinking of higher education or industry, Python is an important asset.
:::

:::{.callout-note icon=false}
## Dynamically interpreted

Python code is run **on the fly**, without needing to be compiled — in contrast to languages that require a non-interactive step converting the program into a binary.

Write code → run it → fix issues directly → rerun, in a **rapid iteration cycle**.

This makes Python a very productive tool for science: you can prototype code quickly.
:::

:::{.callout-tip icon=false}
## Out of the box: a calculator
...
## Open source packages in Python {.scrollable}

:::{.callout-tip icon=false}
## Importing

The strength of Python as a data analysis tool comes from **additional packages**. Loading a package is called *importing*:

```python
import geopandas
```

Shortening the name to save keystrokes is called **aliasing**:
...
The book deliberately does **not** alias, to make code more readable:

`geopandas.read_file(...)`  instead of  `gpd.read_file(...)`

Each new library is introduced within the text of the chapter where it is first required, so the code is threaded pedagogically with the narrative.

Once introduced, later chapters import it at the top with all the other, already-introduced libraries.
:::

## Contextual help

:::{.callout-tip icon=false}
## Help from inside the session

Check what a function does, or how to use it, without leaving Python.
...
JupyterLab also offers a **Contextual Help** box in the launcher: the help of every function where your cursor lands is displayed dynamically.

To print help into the notebook itself:
...
1. It is **widely adopted as an industry standard** — well supported, likely to be maintained
2. It has become a standard in **data science** — foundational projects such as Jupyter publish official containers
3. Because of 1 and 2, building on Docker integrates easily with **cloud services** or local clusters, widening delivery channels
:::

:::{.callout-note icon=false}
## What *is* a container?

A "box" that includes **everything** required to run a certain piece of software. It can be moved from machine to machine and the computations remain **exactly the same, bit by bit**.

When we download a container we are *not* installing software from the usual channels for our platform — we download the software **in the form it was installed when the container was built**.

> **Build once, run everywhere.**

Similar to virtual machines, but far more **lightweight**: faster, less computational power and memory.
:::

## Running the book in a container {.scrollable}

:::{.callout-tip icon=false}
## Pull

The book uses the `gds_env` project — a containerised platform for geographic data science, built on the official Jupyter Docker release, released twice a year.

```shell
docker pull darribas/gds_py:7.0
```

The image is relatively large (~4 GB), so you need the space and a good internet connection. You only need to do this **once**.
:::
...
- `docker run` — we want to *run* a new container
- `--rm` — remove the container when you close it, so every run starts afresh with the exact same setup
- `-ti` — run interactively, not in the background
- `-p 8888:8888` — forward the *port* out to the host machine; **door 1**, lets the Jupyter server inside talk to your browser
- `-v ${PWD}:/home/jovyan/work` — **door 2**; "mounts" the current working directory inside the container, at `work`
- `darribas/gds_py:7.0` — which container to run
:::

## Running the book in a container

![The authentication screen for Jupyter notebooks](../img/gds/jupyter_landing_page.png){width="60%" fig-align="center"}

Copy the token printed in the terminal, paste it in the box, click **Log in**.
...
**Reproducibility requires us to take our tools seriously.**

- Since the onset of computational science, we have sought methods to keep the *code* that implements science tightly linked to the *writing* that explains it → **notebooks**
- Keeping in the spirit of open science, we do better science if our software is **public and open**
- To guarantee our code can in fact be executed, we must **document the environment** it runs in → **Docker**
:::
...
This chapter grounds the previous two into a practical context: how data structures are implemented in Python, and how to interact with them.

Note the emphasis: we discuss how Python represents data **once read**, rather than focusing on specific *file* formats.

Libraries read *any* format into one of a few canonical data structures. Those structures are our **interface with the data** — file formats are secondary. This is the benefit of **abstraction**.
:::

:::{.callout-note icon=false}
## Plan

**Part 1 — the three fundamental structures**

- geographic tables
- surfaces
- spatial graphs

**Part 2 — hybrids**, combinations that depart from the traditional model/structure matchings:
...
Systems using this structure add geography to a *relational database*: PostgreSQL (PostGIS), sqlite (spatialite). Data science languages have their own: R `sf`, Julia `GeoTables.jl`, Python `geopandas`.
:::

:::{.callout-warning icon=false}
## Terminology matters

Throughout the book, consistent with data science and machine learning:
...
There are several Python data structures for spatial graphs, each optimized for a different context. For statistical methods, the most common are **spatial weights matrices** — the subject of Chapter 4.
:::
...
`GeoDataFrame`, `DataArray` and `Graph` are the **conventional pairings** that align data models to data structures.

But others are in active use, and many more are yet to be developed. Many new pairings are driven by **new technology**, enabling approaches that were not possible before, or creating situations (e.g. large datasets) that make the conventional approach limiting.

So: when might it make sense to represent a dataset with a structure that is *not* the obvious initial choice?
:::
...
which equals $100 \times (\texttt{w.s0} / \texttt{w.n}^2)$.
:::
...
- all members of a group are considered "near" one another → weight of one for every pair in the group
- all non-members are disconnected from the group → weight of zero
...
*(Chapters 6 and 7 of the book develop this properly.)*
:::
...
|---|---|---|
| 1 | How do we *think* about geography? | objects, fields, networks |
| 2 | How do we *work* reproducibly? | notebooks, packages, containers |
| 3 | How is geographic data *represented*? | `GeoDataFrame`, `DataArray`, `Graph` |
| 4 | How are *relationships* stored? | spatial weights (`W`) |
:::
...
- the *conceptual* choice (what are the entities? how are they related?) can be made independently of the *computational* one
- recent developments (machine learning, H3, WorldPop) are **breaking down** the traditional pairings
- change-of-support is getting cheap — which makes it *easier*, not less important, to be clear about what you are modelling
...
- Goodchild, Yuan & Cova (2007). "Towards a general theory of geographic representation in GIS." *IJGIS* 21(3)
- Worboys & Duckham (2004). *GIS, A Computing Perspective*. CRC Press
- Gahegan (1999). "Guest Editorial: What is Geocomputation?" *Transactions in GIS* 3

**Reproducibility**

- Rule et al. (2019). "Ten Simple Rules for writing and sharing computational analyses in Jupyter Notebooks." *PLOS Comp Bio* 15(7)
- Rey (2009). "Show me the code: spatial analysis and open source." *J. Geographical Systems* 11(2)

**Spatial weights**

- Anselin & Rey, *Modern Spatial Econometrics in Practice*, ch. 3
- Uitermark & van Meeteren (2021). "Geographical Network Analysis." *TESG* 112
:::
